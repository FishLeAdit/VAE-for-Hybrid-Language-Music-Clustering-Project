...
# -------------------- CONFIG --------------------
MANIFEST_IN = Path("data/fma_manifest_5k_5genres_lyrics_test.csv")  # Input CSV with lyrics
MANIFEST_OUT = Path("data/fma_manifest_5k_5genres_lyrics_updated.csv")  # Output path
...
def main() -> None:
    ...
    for idx, row in tqdm(df_iter.iterrows(), total=len(df_iter), desc="Fetching lyrics"):
        ...
        # Skip if lyrics already found from Genius
        if isinstance(row["lyrics_source"], str) and "genius" in row["lyrics_source"].lower():
            print(f"Skipping {title} by {artist} (lyrics already found from Genius).")
            continue
